...
export function applyConfigToUI(config) {
  ...
  if (socialsContainer && config.developer.socials) {
    ...
    const icons = {
      ...
      coffee: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" width="18" height="18" fill="currentColor"><path d="M192 384h192c53 0 96-43 96-96h32c70.6 0 128-57.4 128-128S574.6 32 504 32H192c-35.3 0-64 28.7-64 64v224c0 35.3 28.7 64 64 64zM504 96c35.3 0 64 28.7 64 64s-28.7 64-64 64h-32V96h32zM128 448h320c17.7 0 32-14.3 32-32s-14.3-32-32-32H128c-17.7 0-32 14.3-32 32s14.3 32 32 32z"/></svg>'
    };

    Object.keys(socials).forEach(key => {
      if (socials[key] && icons[key]) {
        const isCoffee = (key === 'coffee');
        const btn = document.createElement(isCoffee ? 'div' : 'a');
        if (!isCoffee) {
          btn.setAttribute('href', socials[key]);
          btn.setAttribute('target', '_blank');
          btn.setAttribute('rel', 'noopener noreferrer');
        }
        btn.setAttribute('class', `social-btn ${key}-btn`);
        btn.setAttribute('title', isCoffee ? 'Buy me a Coffee' : key.charAt(0).toUpperCase() + key.slice(1));
        ...
        if (isCoffee) {
          ...
          // Wrap QR code in an anchor tag pointing to UPI link
          const qrLink = document.createElement('a');
          qrLink.href = config.developer.upiLink || '#';
          qrLink.className = 'qr-link';

          const qrImg = document.createElement('img');
          qrImg.src = 'buy-me-a-coffee-qr-code.png';
          qrImg.alt = 'Scan to pay using UPI';
          ...
          text.textContent = 'Scan QR to Pay';

          tooltip.appendChild(qrLink);
          tooltip.appendChild(text);

          if (socials[key]) {
            const bmacBtn = document.createElement('a');
            bmacBtn.href = socials[key];
            bmacBtn.className = 'bmac-pay-btn';
            bmacBtn.textContent = 'Pay via UPI App';
            tooltip.appendChild(bmacBtn);
          }
    ...
    document.addEventListener('click', () => {
      const coffeeBtn = socialsContainer.querySelector('.coffee-btn');
      if (coffeeBtn) {
